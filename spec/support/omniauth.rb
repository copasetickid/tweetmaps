...
OmniAuth.config.mock_auth[:twitter_auth] = OmniAuth::AuthHash.new({
  :provider => "twitter",
  :uid => "123456",
  :info => {
...
OmniAuth.config.mock_auth[:twitter_auth_female] = OmniAuth::AuthHash.new({
  :provider => "twitter",
  :uid => "9876543",
  :info => {
    :nickname => "janeqpublic",
    :name => "Jane Q Public",
    :location => "Anytown, USA",
    :image => "http://si0.twimg.com/sticky/default_profile_images/default_profile_2_normal.png",
    :description => "a very normal guy.",
    :urls => {
      :Website => nil,
      :Twitter => "https://twitter.com/janeqpublic"
    }
  },
  :credentials => {
    :token => "a1b2c3d4...", # The OAuth 2.0 access token
